...
class Provisioner(private val app: App) {
    ...
    suspend fun pair(baseUrl: String, masterPassword: String): PairStartRes {
        ...
        Api.ok.newCall(req).execute().use { rsp ->
            if (!rsp.isSuccessful) error("Pair failed: ${rsp.code}")
            val txt = rsp.body?.string() ?: error("Empty body")
            val res = Api.moshi().adapter<PairStartRes>().fromJson(txt) ?: error("Bad JSON")
    ...
    suspend fun wifiConfig(baseUrl: String, ssid: String, pass: String) {
        ...
        Api.ok.newCall(req).execute().use { rsp ->
            if (!rsp.isSuccessful) error("WiFi config failed: ${rsp.code}")
        }
    }
}
